...
(function () {
  ...
  function initCameraFollow() {
    ...
    const api = window.__cameraFollow = {
      ...
      // The world point currently framed — i.e. the one sitting at the SAFE centre,
      // not at the geometric centre of the canvas. It has to be the exact inverse of
      // focusWorld: reading it with one convention and restoring it with the other is
      // what made a recalled viewpoint sit half a safe-area lower than the saved one.
      viewCentre() {
        const c = viewportCenter();
      ...
      glideTo(x, y, pct, opts) {
        ...
        if (travelPx > 4 && auth(z0, zf) < need) {
          // Authority over a leg is env*100*|1/za - 1/zb| in world units, so the whole
          // trip fits when the total variation of 1/z is at least need/(z0*env).
          // Widening to zm gives 2/zm - 1/z0 - 1/zf, hence the apex below. Note the
          // asymmetry that makes the excursion necessary at all: zooming OUT raises
          // 1/z without limit, zooming IN can only ever bring it back down to 1/zf.
          const N = need / env;
          zm = Math.max(CFGV.minZoom, 2 / ((N + 1) / z0 + 1 / zf));
          if (zm >= Math.min(z0, zf)) zm = null;   // nothing to borrow: run flat out
        }
        // The PAN is paid in |d(1/z)|: authority per zoom step is env*|1-k|, which in
        // world units integrates to env*100*|1/za - 1/zb|. So 1/z, not log z, is the
        // currency. Scheduling the pan on log-zoom (the obvious guess) asks the tight
        // end of a zoom-in to carry most of the trip, exactly where authority has
        // collapsed — simulated at a 1478 px shortfall on a 6x zoom-in.
        const iv = (a, b) => Math.abs(1 / b - 1 / a);
        const ss = (u) => u * u * (3 - 2 * u);       // smoothstep: ease in AND out
        let sched;
        if (zm) {
          // A crane move reverses the zoom at the apex, so easing each leg separately
          // parks the zoom rate at zero there — and the pan, being paid in |d(1/z)|,
          // stalls with it. That dead stop is what shows as a seam between the two
          // halves of the flight. So put ONE smoothstep on the PAN across the whole
          // move and let the zoom follow it: the apex becomes a corner (the zoom flips
          // direction crisply, which is what a crane move looks like) while the pan
          // sails through at full speed, easing only at the true start and end.
          const q0 = 1 / z0, qm = 1 / zm, qf = 1 / zf;
          const legA = Math.abs(qm - q0), V = legA + Math.abs(qf - qm);
          const sA = Math.sign(qm - q0), sB = Math.sign(qf - qm);
          sched = (u) => {
            const p = ss(u), done = p * V;
            const q = done <= legA ? q0 + sA * done : qm + sB * (done - legA);
            return { z: 1 / q, p };
          };
        } else {
          // Single leg: no reversal, so the zoom can run on log-zoom (an even zoom
          // rate) and the pan simply inherits its ease through the 1/z conversion.
          const L0 = Math.log(z0), Lf = Math.log(zf), V = iv(z0, zf);
          sched = (u) => {
            const z = Math.exp(L0 + ss(u) * (Lf - L0));
            return { z, p: V > 1e-9 ? iv(z0, z) / V : ss(u) };
          };
        }

        // Duration scales with the zoom SPAN in log terms, which is what the eye reads
        // as "how much zooming", regardless of the 1/z currency used to pay the pan.
        const zSpan = zm ? Math.abs(Math.log(zm / z0)) + Math.abs(Math.log(zf / zm))
                         : Math.abs(Math.log(zf / z0));
        const dur = o.durationMs || Math.max(900, Math.min(3500, 700 + travelPx * 0.6 + zSpan * 900));
